...
export default function BudgetBriefing({ agency }: { agency: Agency }) {
  ...
  return (
    <Card title={`${agency.seal} ${agency.abbrev} Budget Briefing — senior analyst view`}
      ...
      <div style={{ fontSize:17, color:C.textSub, lineHeight:1.7, marginBottom:14 }}>{b.overview}</div>

      <Row>
        <div style={{ flex:1, minWidth:280, background:C.card, border:`1px solid ${C.border}`, borderRadius:10, padding:"12px 14px" }}>
          <div style={{ fontSize:17, fontWeight:700, color:C.text, marginBottom:6 }}>🧱 Funding structure</div>
          <div style={{ fontSize:16, color:C.textSub, lineHeight:1.65 }}>{b.fundingStructure}</div>
        </div>
        <div style={{ flex:1, minWidth:280, background:C.card, border:`1px solid ${C.border}`, borderRadius:10, padding:"12px 14px" }}>
          <div style={{ fontSize:17, fontWeight:700, color:C.text, marginBottom:6 }}>🏛️ Appropriations process</div>
          <div style={{ fontSize:16, color:C.textSub, lineHeight:1.65 }}>{b.appropriations}</div>
        </div>
      </Row>

      {enacted.length > 0 && (
        <>
          <div style={{ fontSize:15, color:C.muted, letterSpacing:"0.08em", margin:"14px 0 8px" }}>
            ...
            {enacted.map(e => (
              <div key={e.fy} style={{ flex:1, minWidth:200, background:C.card, border:`1px solid ${C.border}`, borderRadius:10, padding:"11px 14px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline" }}>
                  ...
                  <span style={{ fontSize:23, fontWeight:800, fontFamily:"var(--font-mono)", color:C.cyan }}>{e.amount}</span>
                </div>
                {e.note && <div style={{ fontSize:15, color:C.muted, marginTop:5, lineHeight:1.5 }}>{e.note}</div>}
              </div>
            ))}
          </div>
        </>
      ...
      <div style={{ marginTop:14, padding:"11px 13px", background:`${C.gold}0f`, border:`1px solid ${C.gold}44`, borderRadius:10 }}>
        <div style={{ fontSize:17, fontWeight:700, color:C.gold, marginBottom:5 }}>⭐ What's unique here</div>
        <div style={{ fontSize:16, color:C.textSub, lineHeight:1.65 }}>{b.uniqueness}</div>
      </div>

      <div style={{ marginTop:14 }}>
        <div style={{ fontSize:15, color:C.muted, letterSpacing:"0.08em", marginBottom:8 }}>SENIOR BUDGET PROFESSIONAL — WHAT I'D TELL YOU OVER COFFEE</div>
        <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
          {b.insights.map((ins, i) => (
            <div key={i} style={{ display:"flex", gap:10, alignItems:"flex-start", padding:"9px 12px",
              ...
              <span style={{ fontSize:16, fontWeight:800, color:C.blue, fontFamily:"var(--font-mono)", flexShrink:0 }}>{String(i + 1).padStart(2, "0")}</span>
              <span style={{ fontSize:16, color:C.textSub, lineHeight:1.6 }}>{ins}</span>
            </div>
          ))}
        </div>
      </div>
